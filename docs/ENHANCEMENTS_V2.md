...
| E-57 | **Attribution panel (SHAP + deploy correlation)** | ✅ | Ch 14, p243–244; Ch 15, p261 | `AttributionPanel` component wired to SHAP API. Shows ranked feature importances with magnitude bars, confidence gauge, deploy correlation (SHA + env in change window). On-demand SHAP analysis via "Run SHAP Analysis" button. Server resource features section for load runs. 4 logic tests. |
| E-58 | **Investigation view (regression timeline)** | ✅ | Ch 15, p260–261 | `/investigation` page with 4 tabs: Change-Point Timeline (run table + mini bar chart), Attribution (wired `AttributionPanel`), Gate Status (run gate results table), Baseline Comparison (active baseline stats + CI + delta from anomaly). Anomaly list sidebar with severity/status filtering. Status update + feedback actions. Added to sidebar nav. 4 logic tests. |
...
| # | Enhancement | Status | Book Ref | Description |
|---|-------------|--------|----------|-------------|
| E-60 | **Decision-driving dashboard (Ch 16 "Practice Four")** | ✅ | Ch 16, p274–276 | `DecisionDashboard` component on Intelligence "Decisions" tab. 7-day review window with deltas-since-last-review (LCP/FCP/CLS/Perf Score), team-segmented KPI table (avg metrics, gate pass rate, trend), action item counter, throughput summary. `computeDelta()` and `classifyTrend()` logic. 5 logic tests. |
| E-61 | **Mobile-first segment (desktop/mobile side-by-side)** | ✅ | Ch 14, p247–248; Ch 15, p264 | `MobileSegmentView` component on Intelligence "Mobile vs Desktop" tab. Device-aware thresholds (mobile LCP good=3000ms vs desktop 2500ms). Side-by-side cards with rating badges, p75 stats, comparison table with gap %. `detectDevice()` handles mobile/phone/moto/nexus patterns. Mobile gap badge. 6 logic tests. |
| E-62 | **Multi-geo dashboard view** | ✅ | Ch 15, p263 | `MultiGeoView` component on Intelligence "Multi-Geo" tab. Region selector, dispatch form, geo runs list, TTFB physics-floor matrix (speed of light in fiber RTT), metric comparison table. `physicsFloorMs()` calculates minimum RTT from distance. Wired to `geo/compare` API endpoint. 4 logic tests. |
| E-63 | **Error bars on all metric charts** | ✅ | Ch 5, p82; App G, p233 | Shared `iqr-utils.ts` with `computeRollingIQR()` and `attachIQRBands()`. IQR shading on trends page (refactored to use shared util), reports page LCP/FCP + Lighthouse trend charts. 16 IQR util tests. |
...
| # | Enhancement | Status | Book Ref | Description |
|---|-------------|--------|----------|-------------|
| E-65 | **Platform health self-monitoring** | ✅ | Ch 14, p245 | `PlatformHealthService` with DB connectivity, run pipeline staleness, worker liveness, schedule adherence checks. Platform Health dashboard tab. |
| E-66 | **On-call rotation management** | ✅ | Ch 16, p270–272 | `OnCallRotationService` with rotation CRUD, member management, paging policies, overrides, event recording. On-Call dashboard tab. |
| E-67 | **Quarterly practice review prompts** | ✅ | Ch 16, p277 | `PracticeReviewService` with review questions, quarter calculation, review CRUD, response submission, scoring (weighted + partial credit). Quarterly Review dashboard tab. |
...
| E-69 | **Sitespeed.io adapter (real implementation)** | ✅ | Ch 5, p77; Ch 15, p263 | Real `SitespeedAdapter` with CLI/Docker execution, `buildArgs()` (device/throttling/viewport), `parseResults()` (walks pages/ dir for browsertime JSON), `aggregateMetrics()` (statistics median or browserScripts fallback), `buildIndividualRuns()`. 21 vitest tests. |
| E-70 | **RUM adapter protocol** | ✅ | Ch 15, p263 | `RumAdapter` interface + `RumAdapterRegistry`. Built-in adapters: `NativeRumAdapter` (pass-through), `SpeedCurveAdapter` (LUX), `DatadogRumAdapter` (view events), `NewRelicRumAdapter` (Browser agent). Common `RumBeacon` type. validate→normalize→toRumEvent pipeline. 38 tests. |

## Priority 10 — Testing & Quality (Ch 5, 12, App G)

| # | Enhancement | Status | Book Ref | Description |
|---|-------------|--------|----------|-------------|
| E-71 | **End-to-end regression lifecycle test** | ✅ | Ch 15, p259–262 | 6-phase integration test: baseline exists → regression arrives → anomaly created → gate fails → anomaly acknowledged/resolved → fix run passes gate. Edge cases for dedup, boundary, null baseline, disabled gate. 15 tests. |
...
_Last updated: 2026-06-02. E-38–62 (except E-64), E-56, E-59, E-63, E-65–67, E-69–72 implemented. Security hardening: global JWT auth guard (`AuthGuard` + `JwtService`), `@Public()` decorator for unauthenticated endpoints (health, RUM ingest, worker), `@Throttle` rate-limiting on auth endpoints (5/min), Helmet security headers, AES-256-GCM encryption for GitHub tokens at rest (`CryptoService`), reset token suppressed in production, login error messages unified, hardcoded DB creds removed in production, user-supplied CrUX API key removed, JSON body limit reduced to 1MB, `ValidationPipe` enabled, dashboard sends `Authorization: Bearer` header. Session upgrade endpoint (`/auth/session-upgrade`) for pre-JWT session migration. 15 security tests. Bug fixes: RUM ingestion missing custom_metrics/build_hash columns; scheduleFlush one-shot bug; aggregateResults double-pick; SQL interval string interpolation → MAKE_INTERVAL; request() header merge spread-order bug; CORS origin comma-separated + wildcard; investigation timeline guard for missing run; 401 auto-redirect to login; project selector defaults to "Select a project" (no auto-select); load page project picker added; demo user password seeding. 1050 API tests, 771 dashboard tests, 205 worker tests passing._
