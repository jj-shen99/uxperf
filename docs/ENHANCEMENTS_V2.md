...
_Last updated: 2026-06-01. E-38–62 (except E-63/E-64), E-56, E-59, E-72 implemented. Security hardening: global JWT auth guard (`AuthGuard` + `JwtService`), `@Public()` decorator for unauthenticated endpoints (health, RUM ingest, worker), `@Throttle` rate-limiting on auth endpoints (5/min), Helmet security headers, AES-256-GCM encryption for GitHub tokens at rest (`CryptoService`), reset token suppressed in production, login error messages unified, hardcoded DB creds removed in production, user-supplied CrUX API key removed, JSON body limit reduced to 1MB, `ValidationPipe` enabled, dashboard sends `Authorization: Bearer` header. Session upgrade endpoint (`/auth/session-upgrade`) for pre-JWT session migration. 15 security tests. Bug fixes: RUM ingestion missing custom_metrics/build_hash columns; scheduleFlush one-shot bug; aggregateResults double-pick; SQL interval string interpolation → MAKE_INTERVAL; request() header merge spread-order bug; CORS origin comma-separated + wildcard; investigation timeline guard for missing run; 401 auto-redirect to login; project selector defaults to "Select a project" (no auto-select); load page project picker added; demo user password seeding. 890 API tests, 660 dashboard tests passing._
