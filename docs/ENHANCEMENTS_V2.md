...
| E-64 | **Audit checklist page (Appendix C)** | � | App C, p221–224 | Interactive audit checklist with 7 categories (measurement hygiene, load, runtime, memory, network, budgets, culture), auto-evaluation from project data, per-item status tracking, weighted scoring, KPI reference guide with `MetricTooltip`, and "no metrics captured" diagnostics. Data extracted to `lib/audit-data.ts` for testability. |
...
_Last updated: 2026-06-05. E-38–67, E-69–72 implemented (including E-64 audit checklist). Security hardening: global JWT auth guard (`AuthGuard` + `JwtService`), `@Public()` decorator for unauthenticated endpoints (health, RUM ingest, worker), `@Throttle` rate-limiting on auth endpoints (5 req/60s, session-upgrade 2 req/60s), Helmet security headers, AES-256-GCM encryption for GitHub tokens at rest (`CryptoService`), startup warnings for missing `JWT_SECRET` / `TOKEN_ENCRYPTION_KEY`, reset token suppressed in production, login error messages unified, UUID validation on session-upgrade, hardcoded DB creds removed in production, JSON body limit 1MB, `ValidationPipe` enabled, dashboard sends `Authorization: Bearer` header, demo passwords overridable via env vars. 1094 API tests, 1153 dashboard tests, 222 worker tests, 135 shared tests passing (2604 total)._
