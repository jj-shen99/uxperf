...
_Last updated: 2026-06-02. E-38–67, E-69–72 implemented (including E-64 audit checklist). Security hardening: global JWT auth guard (`AuthGuard` + `JwtService`), `@Public()` decorator for unauthenticated endpoints (health, RUM ingest, worker), `@Throttle` rate-limiting on auth endpoints (5 req/60s, session-upgrade 2 req/60s), Helmet security headers, AES-256-GCM encryption for GitHub tokens at rest (`CryptoService`), startup warnings for missing `JWT_SECRET` / `TOKEN_ENCRYPTION_KEY`, reset token suppressed in production, login error messages unified, UUID validation on session-upgrade, hardcoded DB creds removed in production, JSON body limit 1MB, `ValidationPipe` enabled, dashboard sends `Authorization: Bearer` header, demo passwords overridable via env vars. 1050 API tests, 771 dashboard tests, 205 worker tests, 135 shared tests passing (2161 total)._
